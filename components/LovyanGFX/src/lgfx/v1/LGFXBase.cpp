...
//drawCircle(x2, y2, 3);
  ...
  void LGFXBase::fillRectInDifference(int32_t x, int32_t y, int32_t w, int32_t h)
  {
    _adjust_abs(x, w);
    _adjust_abs(y, h);
    startWrite();

    uint32_t fill_color = getRawColor();
    uint32_t ret_color;

    for (int i = 0; i < w; i++) {
      for (int j = 0; j < h; j++) {
      /* Get difference */
      if (fill_color >= readPixelRGB(x + i, y + j).RGB888()) {
        ret_color = fill_color - readPixelRGB(x + i, y + j).RGB888();
      }
      else {
        ret_color = readPixelRGB(x + i, y + j).RGB888() - fill_color;
      }
        setRawColor(ret_color);
        drawPixel(x + i, y + j);
      }
    }
    setRawColor(fill_color);

    endWrite();
  }

  void LGFXBase::fillSmoothRoundRectInDifference(int32_t x, int32_t y, int32_t w, int32_t h, int32_t r)
  {
    // startWrite();
    // int32_t xs = 0;
    // int32_t cx = 0;
    ...
    //   uint32_t fill_color = getRawColor();
    //   uint32_t ret_color;
    //   for (int i = 0; i < (2 * (r - cx) + 1 + w); i++) {
    //     /* Get difference */
    //     if (fill_color >= readPixelRGB(x + cx - r + i, y + cy - r).RGB888()) {
    //       ret_color = fill_color - readPixelRGB(x + cx - r + i, y + cy - r).RGB888();
    //     }
    //     else {
    //       ret_color = readPixelRGB(x + cx - r + i, y + cy - r).RGB888() - fill_color;
    //     }
    //     setRawColor(ret_color);
    //     drawPixel(x + cx - r + i, y + cy - r);

    //     /* Get difference */
    //     if (fill_color >= readPixelRGB(x + cx - r + i, y - cy + r + h).RGB888()) {
    //       ret_color = fill_color - readPixelRGB(x + cx - r + i, y - cy + r + h).RGB888();
    //     }
    //     else {
    //       ret_color = readPixelRGB(x + cx - r + i, y - cy + r + h).RGB888() - fill_color;
    //     }
    //     setRawColor(ret_color);
    //     drawPixel(x + cx - r + i, y - cy + r + h);
    //   }
    //   setRawColor(fill_color);
    ...
    uint32_t default_color = getRawColor();
    for (int i = 0; i < w; i++) {
        for (int j = 0; j < h; j++) {
            if (readPixel(x + i, y + j) == TFT_BLACK) {
                setRawColor(default_color);
            }
            else {
                setRawColor(0);
            }
    ...
    //   writeFastHLine(x + cx - r, y - cy + r + h, 2 * (r - cx) + 1 + w);
        ...
        // writeFastHLine(x + cx - r, y + cy - r, 2 * (r - cx) + 1 + w);
        for (int i = 0; i < (2 * (r - cx) + 1 + w); i++) {
            if (readPixel((x + cx - r) + i, y + cy - r) == TFT_BLACK) {
                setRawColor(default_color);
            }
            else {
                setRawColor(0);
        ...
        // writeFastHLine(x + cx - r, y - cy + r + h, 2 * (r - cx) + 1 + w);
        for (int i = 0; i < (2 * (r - cx) + 1 + w); i++) {
            if (readPixel((x + cx - r) + i, y - cy + r + h) == TFT_BLACK) {
                setRawColor(default_color);
            }
            else {
                setRawColor(0);
